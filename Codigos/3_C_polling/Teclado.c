...
/**
 * @brief Configura los puertos GPIO para el funcionamiento del teclado matricial.
 * 
 * Esta función inicializa y configura los puertos GPIO necesarios para el funcionamiento
 * del teclado matricial. Las filas del teclado se configuran como salidas, mientras que las
 * columnas se configuran como entradas con resistencia de pull-down.
 */
void setup() {
    stdio_init_all();
...
/**
 * @brief Obtiene el valor ingresado desde un teclado matricial.
 * 
 * Esta función verifica las teclas presionadas en un teclado matricial y
 * obtiene el valor correspondiente. También obtiene la amplitud, el nivel DC
 * y la frecuencia de la señal ingresada.
 * 
 * @param let Puntero al carácter que representa la tecla presionada.
 * @param ampl Puntero a la variable que almacenará la amplitud de la señal.
 * @param DC Puntero a la variable que almacenará el nivel DC de la señal.
 * @param freq Puntero a la variable que almacenará la frecuencia de la señal.
 */
void get_value(char *let, int *ampl, int *DC, int *freq){
    printf("Presionaste \n");
    // Bucle para recorrer las filas del teclado matricial
    for (int row = 0; row < 4; row++) {
        gpio_put(keypad_rows1[row], 1);
        // Bucle para recorrer las columnas del teclado matricial
        for (int column = 0; column < 4; column++) {

            // Verifica si se presiona una tecla y si el antirrebote está activado
            if (gpio_get(keypad_columns[column]) && AntR1) {
                *let = matrix_keys[row][column];
                tecla_antirrebote[row][column] = true; // Activa el antirrebote para la tecla presionada
                AntR1=false;
        ...
        // Verifica si el antirrebote está desactivado y si ha transcurrido el tiempo especificado
        if(!AntR1){
